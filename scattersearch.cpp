    ...
    while (file.good())
    {
        ...
        else
        {
            ...
            if(line_number == 1)
            {
                ...
                getline(file, value, ',');
                max_strip_height = atoi(value.c_str());
            }
...
void print_one_solution(solution tmp){
    ...
    for (int i = 0; i < (int)tmp.items.size(); i++) {
        cout << "x1: " << tmp.a[i] << ", y1: " << tmp.b[i] << ", x2: " << tmp.a[i] + bs[tmp.items[i]-1].width << ", y2: "  << tmp.b[i] + bs[tmp.items[i]-1].height << "\n";
    }

    if (tmp.fitness > max_strip_height)
    {
        cout << "PACKING FAILED! We can't fit all boxes on the pallet: required height is " <<  tmp.fitness << ", allowed height is " << max_strip_height;
    }
}

std::vector<std::array<int, 4>> get_data_for_output(solution tmp){
    std::vector<std::array<int, 4>> solution_for_output;

    for (int i = 0; i < (int)tmp.items.size(); i++) {
        solution_for_output.push_back({ tmp.a[i], tmp.b[i], tmp.a[i] + bs[tmp.items[i]-1].width, tmp.b[i] + bs[tmp.items[i]-1].height });
    }

    return solution_for_output;
}
